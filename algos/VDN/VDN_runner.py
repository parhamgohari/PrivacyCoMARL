...
class VDN_Runner(Runner_Base):
    def __init__(self, args, env_name, exp_id, seed):
        super(VDN_Runner, self).__init__(args, env_name, exp_id, seed)
            ...
            if min([agent.replay_buffer.current_size for agent in self.agents]) >= (self.args.buffer_size if self.args.use_dp else self.args.batch_size):
                ...
                for agent in self.agents:
                    if self.args.use_secret_sharing:
                        agent.peer2peer_messaging(self.seed, mode = '4. receive the sum of local sums', sender_message = local_sums)
                    else:
                        agent.peer2peer_messaging(self.seed, mode = '4. receive the sum of local sums', sender_message = torch.sum(local_sums, dim = 0))
