...
class Q_network_RNN(nn.Module):
    def __init__(self, args, input_dim):
        ...
        self.fc3 = nn.Linear(args.rnn_hidden_dim, args.action_dim)
    ...
    def forward(self, inputs):
        ...
        x = F.relu(self.fc2(self.rnn_hidden))
        Q = self.fc3(x)
        return Q

    def get_history_encoding(self):
        return self.rnn_hidden.numpy()

    def reset_rnn_hidden(self, hidden_state = None):
        print(f'------hidden state reset to {hidden_state}-----------')
        self.rnn_hidden = hidden_state
