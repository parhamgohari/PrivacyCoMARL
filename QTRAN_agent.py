...
class QTRAN_Base_Agent(object):
    def __init__(self, args, id, seed):
        ...
        if self.use_rnn:
            self.encoding_dim = self.rnn_hidden_dim
        else:
            self.encoding_dim = self.mlp_hidden_dim
    ...
    def choose_action(self, local_obs, avail_a, epsilon, seed):
        ...
        with torch.no_grad():
            ...
            if torch.rand(1).item() < epsilon:
                a = torch.where(avail_a==True)[0][torch.randint(0, sum(avail_a), (1,)).item()].item()
            else:
                a = q_logits.max(dim=-1)[1].item()

            if self.use_rnn:
                self.q_network.hidden = self.q_network.hidden[a].unsqueeze(0).repeat(self.action_dim, 1)
    ...
    def initiate_peer2peer_messaging(self, seed):
        ...
        if self.use_rnn:
            self.q_network.hidden = None
            self.target_q_network.hidden = None
            self.v_network.hidden = None

            q_evals, q_targets, v_evals, h_q_evals, h_q_targets, h_v_evals = [], [], [], [], [], []
            for t in range(self.max_episode_len):  # t=0,1,2,...(episode_len-1)
                # Get the current state of the hidden
                q_network_rnn_state = self.q_network.hidden
                target_q_network_rnn_state = self.target_q_network.hidden

                q_eval_logits, q_target_logits, h_q_logits, h_q_target_logits = [], [], [], []
                # Get the q_values and the corresponding hidden encodings for all actions.
                # Make sure that the hidden state is reset to the original state after each action.
                for a in range(self.action_dim):
                    ...
                    h_q_logits.append(self.q_network.hidden)
                    self.q_network.hidden = q_network_rnn_state
                    ...
                    h_q_target_logits.append(self.target_q_network.hidden)
                    self.target_q_network.hidden = target_q_network_rnn_state
                ...
                # Set the hidden encoding to the encoding of the action taken for both q_network and target_q_network
                reshaped_a = self.batch['a'][:, t].unsqueeze(-1).expand(-1, self.encoding_dim).unsqueeze(-1)
                self.q_network.hidden = torch.gather(h_q_logits_stacked, dim=-1, index=reshaped_a).squeeze(-1)

                if t < self.max_episode_len - 1:
                    reshaped_a_next = self.batch['a'][:, t+1].unsqueeze(-1).expand(-1, self.encoding_dim).unsqueeze(-1)
                    self.target_q_network.hidden = torch.gather(h_q_target_logits_stacked, dim=-1, index=reshaped_a_next).squeeze(-1)


                v_eval = self.v_network(self.inputs[:, t + 1].reshape(-1, self.input_dim)).squeeze(-1)
                h_v_eval = self.v_network.hidden
                v_evals.append(v_eval.reshape(self.current_batch_size, -1))
            ...
            actions_one_hot = torch.eye(self.action_dim)  # shape: (action_dim, action_dim)
            ...
            for t in range(self.max_episode_len):
                ...
                expanded_inputs = self.inputs[:, t].unsqueeze(1).expand(-1, self.action_dim, -1)  # shape: (batch_size, action_dim, input_dim)

                inputs_w_one_hot_a = torch.cat([expanded_inputs, actions_one_hot.unsqueeze(0).expand(self.batch_size, -1, -1)], dim=-1)
                q_eval_logits_t = self.q_network(inputs_w_one_hot_a.view(-1, self.input_dim + self.action_dim)).squeeze(-1)
                q_eval_logits_t = q_eval_logits_t.view(self.batch_size, self.action_dim)
                h_q_logits_t = self.q_network.mlp_hidden.view(self.batch_size, self.action_dim, self.encoding_dim).permute(0, 2, 1)


                next_inputs = self.inputs[:, t + 1].unsqueeze(1).expand(-1, self.action_dim, -1)  # shape: (batch_size, action_dim, input_dim)
                next_input_w_onehot_a = torch.cat([next_inputs, actions_one_hot.unsqueeze(0).expand(self.batch_size, -1, -1)], dim=-1)
                q_target_logits_t = self.target_q_network(next_input_w_onehot_a.view(-1, self.input_dim + self.action_dim)).squeeze(-1)
                q_target_logits_t = q_target_logits_t.view(self.batch_size, self.action_dim)
                h_q_target_logits_t = self.target_q_network.mlp_hidden.view(self.batch_size, self.action_dim, self.encoding_dim).permute(0, 2, 1)
                    
                v_eval_t = self.v_network(self.inputs[:, t + 1].reshape(-1, self.input_dim)).squeeze(-1)
                h_v_eval_t = self.v_network.mlp_hidden

                q_eval_logits.append(q_eval_logits_t)
                q_target_logits.append(q_target_logits_t)
                h_q_logits.append(h_q_logits_t)
                h_q_target_logits.append(h_q_target_logits_t)
                v_evals.append(v_eval_t)
                h_v_evals.append(h_v_eval_t)

            self.q_evals = torch.stack(q_eval_logits, dim=1)
            self.q_targets = torch.stack(q_target_logits, dim=1)
            self.v_evals = torch.stack(v_evals, dim=1)
            self.h_q_evals = torch.stack(h_q_logits, dim=1)
            self.h_q_targets = torch.stack(h_q_target_logits, dim=1)
            self.h_v_evals = torch.stack(h_v_evals, dim=1)
    ...
    def peer2peer_messaging_for_computing_q_jt(self, seed, mode, sender_id = None, sender_message = None):
        if '1' in mode:
            self.message_to_send = torch.zeros((self.current_batch_size, self.max_episode_len, self.encoding_dim, self.n_agents), device=self.device, dtype=torch.float32)
            self.message_to_rece = torch.zeros((self.current_batch_size, self.max_episode_len, self.encoding_dim, self.n_agents), device=self.device, dtype=torch.float32)
    ...
    def peer2peer_messaging_for_computing_y_dqn(self, seed, mode, sender_id = None, sender_message = None):
        if '1' in mode:
            ...
            # self.sum_h_q_targets_excluding_self = self.sum_h_targets[:,:,:self.hidden_dim] - self.h_q_targets.detach()
    ...
    def train(self, total_steps):
        ...
        self.l_td_v = self.contribution_to_q_jt_sum_opt.detach() - self.batch['r'].squeeze(-1) - self.gamma * (1 - self.batch['dw'].squeeze(-1)) * self.v_evals.squeeze(-1)

        masked_l_td = self.l_td * self.batch['active'].squeeze(-1)
        l_td = (masked_l_td ** 2).sum(dim=1) / self.batch['active'].squeeze(-1).sum(dim=1)

        masked_l_opt = self.l_opt * self.batch['active'].squeeze(-1)
        l_opt = (masked_l_opt ** 2).sum(dim=1) / self.batch['active'].squeeze(-1).sum(dim=1)

        masked_l_nopt = self.l_nopt * self.batch['active'].squeeze(-1)
        l_nopt = (masked_l_nopt ** 2).sum(dim=1) / self.batch['active'].squeeze(-1).sum(dim=1)

        masked_l_td_v = self.l_td_v * self.batch['active'].squeeze(-1)
        l_td_v = (masked_l_td_v ** 2).sum(dim=1) / self.batch['active'].squeeze(-1).sum(dim=1)
